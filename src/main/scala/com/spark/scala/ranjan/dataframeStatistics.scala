...
/**
 * Created By Ranjan Kumar,Date-29/12/2019
 */
...
object dataframeStatistics {
  def main(args: Array[String]): Unit = {
    ...
    println("Hi I am starting data frame statistics Programming ")
    ...
    dfQuestions
      .select(avg("score"))
      .show()
    // Max value of score column
    dfQuestions
      .select(max("score"))
      .show()
    // Minimum of score column
    dfQuestions
      .select(min("score"))
      .show()
    ...
    // Correlation between column score and answer_count
    ...
    // Frequent Items in the answer_count dataframe column
...
//Stratified sampling using sampleBy - For this we will use some couple of steps as below.
    ...
    //we can also verify the quantiles statistics above using Spark SQL as follows:
